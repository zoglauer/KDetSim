{
    gROOT->Reset();
    gSystem.Load("/home/bojan/KDetSim/KDetSimDist_gauss_beam/lib/KDetSim.sl");
  ...
  */
    ...
    /*  for (int iStep=0; iStep<2; iStep++)
  {
    ...
    if (makeScan) {
        ...
        hCCEslice->Draw();
        if (savePlot)
            c1->Print(Form("plots/slice_%s_y_%.0lf.png", fileName, substrateThickness - 50.0));

        hCCE->Draw("colz");
        if (savePlot)
            c1->Print(Form("plots/%s.png", fileName));
    }
}
